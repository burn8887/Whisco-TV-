...
*Last updated: 2026-09-10 (NEW CREDENTIALS: Vercel API token .keys/vercel_token.txt (project-scoped, analytics API); Google service account .keys/gsc-service-account.json = whisco-agent@dulcet-record-441513-s1.iam.gserviceaccount.com (GSC Full both properties LIVE; Play Developer API granted, propagating; enable more Google APIs on project dulcet-record-441513-s1 as needed). New live features: Tonight on Whisco (src/components/tonight/), guides mesh+JSON-LD, favicon=mascot. Trending/shelves: modern-first rules in maintenance cron + cached.ts. demo user deleted. Previous: 2026-09-07-c Apple/ASC registry RESTORED after doc-rot found by Claude freshness test; incident-triage + community-digest workflows added to automations list. Previous: iOS SUBMITTED FOR APP STORE REVIEW — WAITING_FOR_REVIEW since 08:05 UTC. Plus: standing-PAT policy; WHISCO_TV_BILLING.md + billing-reminder workflow; Apple/ASC credentials registered; Neon usage-based ~$5-15/mo). Companion to `WHISCO_TV_PROJECT_HANDOVER.md` (read BOTH).*
