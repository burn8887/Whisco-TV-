...
## 2026-09-14 (Monday) — AGENT SUCCESSION COMPLETE + GEO-EXPOSURE FINDING
- **Handover done.** New engineering agent cloned repo (main @ 6d6d86f — note: the takeover brief cites 00e7b4e; two newer commits have landed since, so the brief's §4 snapshot is 2 commits stale). Read CLAUDE_TAKEOVER_BRIEF.md + this handover end to end. Bootstrap clean (`npm install` + `prisma generate`); note node_modules/.git-config are stripped every turn by the workspace snapshot, so the §2 bootstrap must be re-run each session.
- **Credentials restored** from the succession pack to /home/user/.keys/ (dir 700, files 600) + iptv-app/.env (gitignored, `git status` clean); upload deleted after verify. All five RESTORE_README checks PASS: ASC JWT 200, GSC token refresh OK (SA sees both properties), Vercel project API 200 (team-scoped; /v2/user 404 is expected for a project-scoped token), Prisma→Neon connect OK, GitHub PAT read+`push --dry-run` OK ("Everything up-to-date", local==origin). No secret was committed or echoed.
- **Apple**: version 1.0 + reviewSubmission c80e30c4 both still **WAITING_FOR_REVIEW** (submitted 2026-09-11T14:54:07Z → ~57h elapsed at check time, past the ~48h [EST] SLA). Build 1.0(5) id 8c79fe28, processingState VALID, not expired. One live version only. No unresolved issues visible; legacy `appStoreVersionSubmissions` returns 404 (expected under reviewSubmissions). Day-14 follow-up (case 102956986441) still gated on state being unchanged on Sep 16.
- **Guides indexing IMPROVED**: 3 of the 5 AdSense-gate URLs are now **"Submitted and indexed"** (turkish-series-guide, free-tv-for-expats-gulf, pakistani-dramas-guide — crawled Aug 30–Sep 3). Still **"URL is unknown to Google"**: `/guides/bollywood-classics-free` and `/new`. GSC sitemap lastDownloaded 2026-09-13, 0 errors. 7-day totals: "whisco tv" = 72 clicks/84 impressions at position 1. AdSense request (Sep 17–19) needs the remaining 2 indexed.
- **Workflows 9/9 GREEN** (uptime #1033, channel-health #113, dizi #28, content-discovery #7, weekly-maintenance #4, billing #1, community-digest #3, incident-triage #235, content-qa #1); zero non-success in the last 40 runs. `DATABASE_URL` Actions secret IS now set (2026-09-13) in addition to CRON_SECRET — the content-qa action item from the brief is closed.
- **Site health OK**: /api/health `{"status":"ok","warnings":[]}`, DB 1.3s, 598 active channels / 16,859 active titles, channels+vod checked 4h ago. Direct DB read: 611 channels (598 active), 16,919 titles (16,859 active), 10,769 episodes.
- **NEW FINDING — geo exposure (see WHISCO_TV_GEO_EXPOSURE_FINDING.md, filed this date)**: Leyla, Sahipsizler, Kızılcık Şerbeti (all three = the original Leyla-incident titles) plus "Security" are `isActive=true, lastStatus=ok` while **verified GCC-blocked** on YouTube; /watch/episode pages serve the blocked embeds and all three are in sitemap.xml. Root cause: the current health checker returns `invalid` (not `geo`) for a GCC block, so **0 rows** carry a `geo` status, `maintenance`'s `geoHidden` KPI is hard-wired to report 0, and the auto-restore path flips blocked titles back on. Sweep latency compounds it: 250 titles/run × 4/day ≈ 17-day full cycle; 8,495 active titles currently unchecked >7d. Random 27-title sample found 1 blocked (3.7% ±7.1pp [EST]); a targeted 24-title sample found 0 — so the count is unknown but small, NOT a return of the original 2,414. Fix proposal: hide the 4 confirmed titles now (data-only, freeze-safe), and on the Sep 20 deploy write a real `geo` status again, report real status splits in maintenance, and cut the sweep cycle under ~7 days. Awaiting founder go.
- **Issue #5** (Basil brief, week 2026-09-12) still open: Expat.com posted (link under review), 4 Reddit mod-mails pending, Turkish §4a available, WhatsApp held for app launch, follow-up kit C1–C4 pre-approved. No new mod replies.

## 2026-09-14 (later) — GEO AUDIT EXECUTED + SEO HOST FINDING + PLAYS TATE CHECK
- **Geo remediation done (founder Option 1).** Fifth blocked title found and hidden: `carpinti` (Çarpıntı) — added 2026-08-31 as "geo-verified", GCC-blocked now, which proves geo status DRIFTS and must be re-checked continuously. Hidden set now: leyla, sahipsizler, kizilcik-serbeti, security, carpinti. DB: 5 geo rows, 65 inactive titles.
- **Method correction (important, do not repeat the mistake):** a per-title brute force over all ~8,400 stale titles is IMPOSSIBLE — YouTube 429s the watch page after ~340 rapid fetches from one IP (reproduced on both sandbox and GitHub runner). But **oEmbed is a separate, tolerant service**: 8,378 titles -> 110 channels in 30 seconds. Channel-level narrowing works as a TARGETING filter, but a channel verdict does NOT certify its titles: "Shout! Studios" (428 titles) probes ok while its title `security` is GCC-blocked => **geo restriction is per-video**. Every write therefore still required per-title verification. Audit tooling: `scripts/geo_channel_audit.mjs` + manually-dispatched `.github/workflows/geo-reaudit.yml` (inputs: dry_run; artifact geo-audit-results). Delete both once the check-vod fix is live.
- **16 DEAD VIDEOS found** as a by-product of the oEmbed liveness step (oEmbed 401/403/404 = gone or non-embeddable): 15 DW Documentary titles + `ar-d3dce2c487` (وراء الشمس). LEFT LIVE and deliberately NOT refreshed in the rotation so the production sweep reaches them early; REPORTED to founder, awaiting a decision on hiding them now (same reviewer-risk class as a geo-block). Full slug list in the finding doc.
- **Catalog-wide geo count still UNDETERMINED [EST]** — ~990 titles probed directly today found 0 blocked; one 27-title random sample found 1 (Security). Low base rate, with per-video exceptions a channel scan cannot rule out.
- **Code fix staged:** branch `geo-health-fix` @ 48afa66 (geo/geo-partial statuses, confirm-before-restore, real maintenance KPI split + alarms, BATCH_SIZE 250->700 = full pass <5 days). tsc 0, eslint clean. RECOMMENDATION SENT TO FOUNDER: deploy this now as a cron-route-only change (no page/UI/structure => outside the freeze's intent) so the sweep becomes the continuous per-title geo audit and completes a pass inside the AdSense window. Otherwise it ships Sep 20 as originally agreed.
- **NEW SEO FINDING — canonical/sitemap host mismatch:** the site is served on **www** and the apex **308-redirects to www**, but EVERY canonical tag, the sitemap (2,727 URLs), robots.txt's Sitemap line, feed.xml, JSON-LD and the share/copy-message links all used the **apex** `https://whisco.tv`. So all 2,727 sitemap URLs cost a redirect hop and the canonical disagreed with the sitemap. GSC confirms `userCanonical = whisco.tv` while `googleCanonical = www.whisco.tv` for the pages Google did index. Suspected contributor to `/new` and `/guides/bollywood-classics-free` still being "unknown to Google" (which gates the AdSense request). Fixed on branch **`canonical-host-fix` @ 9a05a2a** (10 files, apex->www literals only; emails untouched; tsc 0). **NOT DEPLOYED** — held for founder approval because of the Sep 16-19 freeze. Separately: founder should use GSC "Request Indexing" on those 2 URLs (fastest lever).
- **Play state read via API (2026-09-14):** tracks — production: none; beta/alpha: none; internal: versionCode 5 ("5 (1.0.0)", completed); **closed track named "whisco.tv test " (trailing space): versionCode 6 ("6 (1.0.0)", completed)**. Testers — only that closed track has a list: Google Group `testers-community@googlegroups.com` (production testers endpoint returns 400). **The Play API does NOT expose the 14-day opt-in clock** — project log says clock 11/14 on Sep 10 / "~day 13/14" on Sep 12, implying day 14 ≈ Sep 13 [EST]. Founder must confirm on Play Console Dashboard (the "Apply for production" option appears there, and the eligibility date is set by the 12th tester's opt-in, not the track creation date).
- **NEW PLAY RISK — target API level:** from **31 Aug 2026** new apps and updates must target **Android 16 (API 36)**. Whisco mobile is Expo ~57.0.15 / RN 0.86.2 and the repo contains **no explicit `targetSdkVersion`** (no `android/` dir — EAS prebuild manages it). Expo SDK 53 defaulted to API 35; SDK 57's default is UNCONFIRMED. Check the warning banner on the Play Console track page before applying; if it warns, add the `expo-build-properties` plugin with `android.targetSdkVersion: 36` and rebuild (EAS handles it).
- Workspace hygiene: temporary scripts removed from the repo tree; `_geo_reaudit.mjs`/`_channel_map.mjs` were scratch only.

## 2026-09-14 (workspace) — STORAGE: repo is now a SHALLOW clone + measured the real weight
- **The scary number is not real.** Workspace reads 1.7GB but `node_modules` (777MB) and `.cache`/`.npm` are EXCLUDED from snapshots. **True snapshot weight: 92MB against a ~128MB cap (~36MB headroom).** Do not panic-prune.
- **Repo converted to a shallow clone** (`git fetch --depth 1` + `reflog expire` + `gc --prune=now`; `--aggressive` skipped — no benefit, more time/risk). `.git` 29MB -> 22MB. **Saved only 7MB, not the ~22MB a shallow conversion usually implies**: this repo's `.git` bulk is the *compressed current content* (the 39MB of `prisma/*.json` compress to ~half the pack), not old history — there was little history to drop.
- **DO NOT MOVE `prisma/*.json` (39MB) OUT OF THE REPO.** They are load-bearing: `prisma/seed.ts` reads `live_channels.json`, `vod_titles.json`, `vod_youtube.json`, `dizi_series.json`, `arabic_series.json`, and `scripts/restore_from_backup.mjs` reads `backup_channels.json`, `backup_titles_flat.json`, `backup_series_full.json`. Moving them breaks `npm run db:seed` and the DR restore. They are the disaster-recovery source of truth — same tier as the DR doc itself.
- **Bootstrap ORDER MATTERS in a snapshot-stripped repo:** `.git/config` + `.git/refs` are stripped every turn, so `git config user.email ...` FAILS with "not in a git directory" until `mkdir -p .git/refs/heads .git/refs/tags` has run. Correct order: `mkdir -p .git/refs/heads .git/refs/tags` FIRST, then `git config`, then `git remote add`, then `git fetch`. Verified end-to-end this session: fetch + `reset --hard origin/main` + push all work fine on a shallow clone. Local branches lost by the strip are recreated with `git branch <name> origin/<name>` (did this for `where-pages`).
- Post-cleanup composition: content 70MB (`prisma` 39 / `docs` 17 incl. a 12MB dead Grok prototype / `public` 7.3 production / `design-drop` 6.6) + `.git` 22MB + keys/tooling 0.5MB.
- **Proposed diet (needs founder go, low priority — there is headroom):** (1) compress the ~15 duplicate 1024px launcher PNGs inside `docs/experiments/grok-app/native-v2/` — a concluded experiment, referenced by no code — worth ~10MB; (2) move `design-drop/` (6.6MB) to a private `whisco-assets` repo AFTER the Sep 20-Oct 3 design sprint that needs it. Together ~75MB end state. No history rewrite needed.
- **Standing policy (founder-approved 2026-09-14):** never commit a new binary >1MB to the main repo without weighing the clone cost — assets repo or founder-side storage per the storage protocol.
- No production impact from any of this: no deploy, no DB writes, no store/GSC API calls. AdSense recrawl, Apple review and Play console untouched.

## 2026-09-14 (night) — FOUR FOUNDER DECISIONS EXECUTED: geo fix + dead videos + canonical fix DEPLOYED, Play walkthrough delivered
- **(a) geo-health-fix DEPLOYED** (`623182a` merge; deploy `c6b5eba` READY). First 3 live sweeps: 231/177/179 titles checked; ok 181/177/178; **geo 33/0/0**; invalid 16/0/0; unknown 1/0/1; restored 4/0/0; newlyHidden 0/0/0. **NO OVER-HIDING** (runs 2-3 fully clean) and **`active + geo = 0` verified** (no geo-blocked title viewer-facing). The 33 geo titles were ALREADY hidden as `invalid` — the original bug in action (invalid 122→90 as they were reclassified). They are Forensic Files / Untold Stories of the ER / brand-catalog titles = the FilmRise/Shout pattern. Two wins: the catalogue state is now honest, and those 33 now fall under the new confirm-before-restore guard that would have blocked the Leyla-style silent restore.
- **CORRECTION to my own claim:** the sweep is **NOT <5 days**. Run 1 completed 231 of 700 requested; the **230s time budget** is the bottleneck, not the batch. `BATCH_SIZE` reverted 700→250 (`2bf80b8`) to stop wasting Neon egress. MEASURED sweep rate ~1,150 titles/day → **full pass ~15 days** (old code comment claimed 12; I claimed <5 — both wrong). The real speed-up: cache channel→geo verdicts within a run (brand catalogues share one verdict) → a 250-title run spanning ~50 channels needs ~50 probes not 250. Designed, NOT built, proposed for the Sep 20 sprint.
- **(b) 16 dead videos HIDDEN** (founder-approved scope extension), each re-verified with a fresh oEmbed probe immediately before the write; written `isActive=false, lastStatus="invalid"` (kept honest — NOT geo). Full slug list in WHISCO_TV_GEO_EXPOSURE_FINDING.md. Run 1 independently re-confirmed exactly 16 invalid = free cross-validation. **Lesson:** `ar-d3dce2c487` is a SERIES with `streamUrl: null` — video IDs live on EPISODES; a streamUrl-only check skips it (mine did). Always resolve SERIES video IDs from episodes. Verified 3 of 30 episodes all dead, none live.
- **(c) canonical-host-fix DEPLOYED** (same deploy). Live-verified: `/new` + `/guides/bollywood-classics-free` canonicals now www; sitemap 2,718 URLs all www; robots.txt Sitemap www; title canonical + JSON-LD www. **GSC sitemap re-submitted** (HTTP 204, lastSubmitted 2026-09-14T01:25:25Z, 0 errors/0 warnings). **IndexNow pinged** (HTTP 200, 7 URLs). REMAINING ON FOUNDER: manual Request Indexing on the 2 stuck URLs.
- **(d) Play:** closed track `whisco.tv test ` = versionCode 6, completed; testers only via Google Group `testers-community@googlegroups.com`; bundles vc5+vc6. **Play API cannot expose the 14-day clock or the target-API banner** (bundles return only versionCode/sha1/sha256 — verified). **Target API 36 expected COMPLIANT: Expo's table lists SDK 57 → compileSdk 36 / targetSdk 36**, and the app is Expo ~57.0.15; Console banner is the authoritative check. Full click-by-click walkthrough + copy-ready answers (all ≤300 chars, verified) at **docs/business/Whisco_TV_Play_Production_Application.md**.
- **Catalog (measured, post-writes):** ok 16,032 | unknown 757 | invalid 90 | geo 33 | duplicate 3 → active 16,838 / inactive 81.
- **STANDING PROCESS CHANGE:** `lastStatus` distribution check added PERMANENTLY to the daily watch list (brief §7 item 7) — report the split and whether it MOVED, plus assert `active + geo == 0`. The never-fails-KPI failure mode is twice-proven.
- Merge note for future sessions: `.git/config` stripped between turns means `git merge` fails with "Committer identity unknown" AND shallow clones cannot merge branches without a common ancestor — `git fetch --deepen=40` first, then merge, then re-shallow (`--depth 1` + `gc --prune=now`, proven safe, `.git` back to 22MB).

## 2026-09-14 (late) — ⚠️ REGRESSION FOUND, ROOT-CAUSED, FIXED, VERIFIED
- **Sweep run #1 RESTORED three GCC-blocked titles to the live site** (Leyla, Sahipsizler, Kızılcık Şerbeti; written `isActive=true, status=ok` at 01:12). My claim that they were "protected by the new confirm-before-restore guard" was WRONG — the guard was defeated by the bug it was meant to guard, because both shared the same false assumption.
- **ROOT CAUSE (core knowledge — do not forget this):** YouTube's watch page carries `availableCountries` **only when the video is NOT playable from the requesting region**. A list-less page means "playable HERE" and says NOTHING about the Gulf. The code read `if (!m) return "ok"` = "worldwide available" → restored blocked titles. **Proof:** a GitHub verify run probed 4 titles × 3 episodes = 12 probes, **zero lists returned — including the control Kuruluş Osman, which is known GCC-available.** So list-absence is ENVIRONMENTAL, never evidence.
- **FIX `1975f5c` deployed + verified:** restore ONLY on positive evidence — a list that EXISTS and CONTAINS a GCC country. Blocked reading → `geo`; list-less/unreadable → `unknown` and STAYS HIDDEN. Applied to **every inactive YouTube title**, not just geo-status ones (the old code mislabelled geo blocks as `invalid`, which the same path would restore). Accepted trade-off: false negatives (a genuinely-unblocked title stays hidden until an audit verifies it) — the correct side to fail on.
- **Verification run #4 (live code):** checked 250/250, ok 242, invalid 1, unknown 0, geo 7, **restored 0** (buggy run had 4), newlyHidden 0, totalInactive 80. `active + geo = 0` holds. The 3 titles remain hidden with `lastStatus=geo`.
- **The 3 re-hidden at 01:29** on earlier same-day **list-backed** evidence (list present, 223/227/231 countries, zero GCC in each) + they match the project's own August audit. A fresh probe was impossible then: sandbox IP HTTP-429 blocked, GitHub vantage cannot see a list for these titles.
- **STRUCTURAL LIMITATION (on the record):** a vantage can only see a block it is itself subject to. **The production sweep runs from a US region and structurally CANNOT detect Gulf-only blocks** (titles available in the US but not the GCC — exactly what these three are). It CAN detect titles blocked in both places (most FilmRise/Shout), which is why the August audit found 2,414. So: **the sweep is a regression GUARD, not a detector.** Detection needs a vantage where the titles are blocked — the sandbox vantage DID see all three Gulf-only blocks, so audits from there work but must be chunked (IP gets 429 after a few hundred probes). **Paid option for founder decision:** a GCC-region proxy/VPS (~$5–10/mo [EST]) would let the audit ask from inside the market we serve. Recurring spend → needs a decision.
- **InnerTube region-parameter approach TESTED AND REJECTED:** unauthenticated calls returned UNPLAYABLE for every title including the known-good control, so it needs visitor-data/PO-token handling. Recorded so nobody re-explores it blindly.
