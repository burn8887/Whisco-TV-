...
*Last updated: 2026-09-07-b (WORKSPACE BUDGET INCIDENT resolved: over-budget snapshot dropped 158 files incl. local repo commits — GitHub was ahead of local; fixed by `git reset --hard origin/main`. LESSON/RULE: GitHub is ALWAYS source of truth; at any session start compare local HEAD vs remote HEAD and reset to remote if behind; keep workspace <100MB — raw experiment extractions get archived to repo then DELETED locally same session. Current state: Home Time live on /live; About = Gulf-household framing; Grok experiment v1+v2 archived in docs/experiments/grok-app (v1.1 shortlist: kids face, zapper, Gulf-first onboarding); Filmhub call WEDNESDAY; Apple day 6 WAITING_FOR_REVIEW (status inquiry at day 7); Play day 6/14; budget $3k/mo tiered per Operating Rules v2 §3.)*
