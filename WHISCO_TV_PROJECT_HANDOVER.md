...
*Last updated: 2026-09-02 (Filmhub called with invitation; standing-PAT policy adopted; billing ledger + monthly reminder live; Neon ~$10 charge audited = correct usage-based pricing; TestFlight build live. Previous: 2026-09-01 INCIDENT RESOLVED: 15:42 UTC uptime-monitor failure was a GitHub-runner DNS blip (curl exit 6), site never down — monitor hardened with 3x retry + DoH DNS cross-check + mobile API v1 endpoints now monitored; 26 closed-testing testers active; iOS: BUILD SUCCEEDED — signed .ipa on EAS (build 563ed5f9, buildNumber 5); cert 3D2AQ39PD3 + profile GJ7634S5R8 + bundleId CGDK5D6GU4 all created via ASC API key B279KL3Y3K (p8 in /home/user/.keys, gitignored); LOCAL credentials source (credentials.json + credentials/ in whisco-mobile, gitignored); TESTFLIGHT LIVE: ascAppId 6807647992, build 1.0.0 (5) VALID + IN_BETA_TESTING, internal group "Whisco Internal" 03e7edb4 hasAccessToAllBuilds, tester burn8887@gmail.com added; App Store review submission = next). Purpose: allow any future conversation/agent to resume this project with zero context loss. Read fully before acting.*
