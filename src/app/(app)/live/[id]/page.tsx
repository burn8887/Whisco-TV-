...
import { getFullUser, isSubActive, hasTierAccess } from "@/lib/access";
...
export default async function ChannelPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const userTier = user?.subscription?.plan.channelAccess ?? "BASIC";
  const unlocked = active && hasTierAccess(userTier, channel.tier);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      {unlocked ? (
        ...
        <div className="relative w-full aspect-video rounded-xl overflow-hidden bg-zinc-900 ring-1 ring-white/10 grid place-items-center">
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={channel.logoUrl} alt="" className="absolute inset-0 w-full h-full object-cover blur-xl opacity-30" />
          <div className="relative text-center px-6">
            <Lock size={36} className="mx-auto mb-4 text-zinc-400" />
            <h2 className="text-xl font-bold mb-2">
              {!active ? "Subscription required" : `Upgrade to ${channel.tier} to watch this channel`}
            </h2>
            <p className="text-zinc-400 text-sm mb-6 max-w-md mx-auto">
              {channel.name} is part of our {channel.tier.toLowerCase()} channel tier.
            </p>
            <Link href="/pricing" className="inline-block px-6 py-3 rounded-full font-semibold bg-gradient-to-r from-orange-500 to-pink-600">
              View Plans
            </Link>
          </div>
        </div>
      ...
      <div className="mt-6 flex items-start gap-4">
        ...
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="text-[11px] font-bold text-zinc-300 bg-white/10 px-2 py-0.5 rounded-full">{channel.tier}</span>
          </div>
