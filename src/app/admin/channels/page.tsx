...
import { Plus, Pencil, Trash2, CheckCircle2, XCircle, Clock, ChevronLeft, ChevronRight } from "lucide-react";

export const dynamic = "force-dynamic";

const PAGE_SIZE = 150;

export default async function AdminChannelsPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; page?: string }> }) {
  ...
  const [channels, total, inactiveCount, filteredCount] = await Promise.all([
    ...
    <div className="p-8">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-4 mt-6">
          <Link
            href={{ pathname: "/admin/channels", query: { ...(sp.q ? { q: sp.q } : {}), ...(sp.status ? { status: sp.status } : {}), page: String(Math.max(1, page - 1)) } }}
          ...
          <Link
            href={{ pathname: "/admin/channels", query: { ...(sp.q ? { q: sp.q } : {}), ...(sp.status ? { status: sp.status } : {}), page: String(Math.min(totalPages, page + 1)) } }}
