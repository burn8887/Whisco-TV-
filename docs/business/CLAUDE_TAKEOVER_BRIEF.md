...
- **AdSense (G1) — CORRECTED 2026-09-15, now API-driven:** the site is **NOT under review** and never was. `accounts/pub-7207533964778777` → account state **READY**, site `whisco.tv` state **NEEDS_ATTENTION**, alert `adsense-onboarding-incomplete`: *"add your payment info and connect your site"*. Two founder-only onboarding tasks stand between us and a review; the guide-indexing gate is secondary, not the blocker. Check with `python3 asc/check_adsense.py` (read-only OAuth token in `.keys/`, never committed). A second alert `ua-conflict-policy-update` is the standard Ukraine notice sent to every publisher — informational, no action.
